...
const AI_MODELS = [
  "mistral/ministral-3b", // inp: 0.04/M - out: 0.04/M - ctx wnd: 128K
  "google/gemini-2.0-flash-lite", // inp: 0.07/M - out: 0.30/M - ctx wnd: 1M
  "google/gemini-2.5-flash-lite", // inp: 0.10/M - out: 0.40/M - ctx wnd: 1M
  "openai/gpt-4.1-nano", // inp: 0.10/M - out: 0.40/M - ctx wnd: 1M
  "openai/gpt-4o-mini", // inp: 0.15/M - out: 0.60/M - ctx wnd: 128K
  "openai/gpt-5-nano", // inp: 0.05/M - out: 0.40/M - ctx wnd: 400K
  "zai/glm-4.7-flashx", // inp: 0.06/M - out: 0.40/M - ctx wnd: 200K
  "xai/grok-4.1-fast-non-reasoning", // inp: 0.20/M - out: 0.50/M - ctx wnd: 2M
  "xai/grok-4-fast-non-reasoning", // inp: 0.20/M - out: 0.50/M - ctx wnd: 2M
];
...
const getNextModel = () => {
  ...
  logger.info({ model, currentModelIndex }, "[get-next-model] Using model");
  return model;
};
...
function isModelFallbackError(error: unknown): boolean {
  ...
  // GatewayInternalServerError wraps APICallError in .cause
  if (
    error instanceof Error &&
    error.cause !== undefined &&
    APICallError.isInstance(error.cause)
  ) {
    return (error.cause as APICallError).isRetryable;
  }
  return false;
}
...
async function identifyTechnologies(
  ...
  const packageSet = new Set(allPackages);

  let lastError: unknown;

  for (let attempt = 0; attempt < MAX_MODEL_ATTEMPTS; attempt++) {
    const model = getNextModel();
    logger.info({ model, attempt }, "[identify-technologies] Using model");

    try {
      const { output } = await generateText({
        model,
        system: IDENTIFY_PROMPT,
        prompt: JSON.stringify({ packages: allPackages }, null, 2),
      ...
      logger.error(
        { model, error },
        "Non-retryable error identifying technologies",
      );
      throw error;
    }
  }
